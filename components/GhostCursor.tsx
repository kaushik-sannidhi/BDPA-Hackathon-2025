...
const GhostCursor = ({
  ...
  const trailBufferRef = useRef<THREE.Vector2[]>([]);
  ...
  const pixelBudget = targetPixels ?? (isTouchDevice ? 0.9e6 : 1.3e6);
  const fadeDelay = fadeDelayMs ?? (isTouchDevice ? 500 : 1000);
  const fadeDuration = fadeDurationMs ?? (isTouchDevice ? 1000 : 1500);
  ...
  useEffect(() => {
    ...
    trailBufferRef.current = Array.from({ length: maxTrail }, () => new THREE.Vector2(0.5, 0.5));
    headRef.current = 0;
    ...
    const material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        iPrevMouse: { value: trailBufferRef.current.map((v) => v.clone()) },
        iOpacity: { value: 1.0 },
        iScale: { value: 1.0 },
    ...
    const resize = () => {
      ...
      const currentDPR =
        typeof window !== "undefined" ? Math.min(window.devicePixelRatio || 1, maxDevicePixelRatio) : maxDevicePixelRatio;

      const neededPixels = cssWidth * cssHeight * currentDPR * currentDPR;
      const scale =
        neededPixels <= pixelBudget
          ? 1
          : Math.max(0.5, Math.min(1, Math.sqrt(pixelBudget / Math.max(1, neededPixels))));
      ...
      const widthPx = Math.max(1, Math.floor(cssWidth * pixelRatio));
      const heightPx = Math.max(1, Math.floor(cssHeight * pixelRatio));

      material.uniforms.iResolution.value.set(widthPx, heightPx, 1);
      material.uniforms.iScale.value = calculateScale(host);
      bloomPass.setSize(widthPx, heightPx);
    };
    ...
    const animate = () => {
      ...
      const N = trailBufferRef.current.length;
      headRef.current = (headRef.current + 1) % N;
      trailBufferRef.current[headRef.current].copy(mat.uniforms.iMouse.value);

      const array = mat.uniforms.iPrevMouse.value as THREE.Vector2[];
      for (let i = 0; i < N; i++) {
        const srcIdx = (headRef.current - i + N) % N;
        array[i].copy(trailBufferRef.current[srcIdx]);
      }
    ...
    const onPointerMove = (event: PointerEvent) => {
      ...
      const y = THREE.MathUtils.clamp(1 - (event.clientY - rect.top) / Math.max(1, rect.height), 0, 1);
  ...
  useEffect(() => {
    if (materialRef.current) {
      ...
      materialRef.current.uniforms.iBaseColor.value.set(base.r, base.g, base.b);
    }
  }, [color]);
